...
Violations appear as red squiggles in the editor and as entries in the Problems panel — no configuration required beyond having `strata` installed.
...
Open any project that has a `STRATA.md` manifest. Violations appear automatically. No commands to run, no settings to configure.
...
The extension cannot find `strata`. Install it with `go install` (see above) and ensure `$HOME/go/bin` is in your `PATH`. If you installed it elsewhere, add that directory to `PATH` instead.
